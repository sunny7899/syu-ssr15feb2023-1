...
export class CollegeConComponent implements OnInit {
  ...
  public submitForm1() {
    ...
    this.http
      .get(
        `https://www.selectyouruniversity.com/api/response.php?cCity=${cCity}&cCourse=${cCourse}&cCandidateName=${cCandidateName}&cEmail=${cEmail}&cMobile=${cMobile}&qeducation=${qeducation}&cLinkName=${this.currentUrl}&section=insertdetails`
      )
      .subscribe((res) => {
        console.log('res', res);
        this.nsrNo = res;
  ...
  ngOnInit() {
    ...
    this.api
      .getAll(`section=colunimaininfo&coluni=${this.url[2]}`)
    ...
    this.api.get(`syudadmissionmaster/${url}`).subscribe((res: any) => {
      ...
      if (res.isError) {
        ...
        res.object.forEach((evn: any) => {
          this.eventaddmission.push({
            Events: evn.cEvents,
            'Important Dates': evn.cImportantDates,
          });
          this.courseaddmission.push({
            Course: 'Test',
            'Exam Accepted': evn.cExamAccepted,
          });
        });
