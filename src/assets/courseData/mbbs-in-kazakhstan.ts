export const mbbsinkazakhstan= {
    "Title": "MBBS in Kazakhstan",
    "shortTitle": "MBBS in Kazakhstan",
    "location": " ",
    "type": "bb",
    "rating": "vv",
    "shareLink": "www.google.com",
    "Duration":"6 Years with internship",
    "url":"mbbs-in-kazakhstan",
    "info": [
        {
            type: 'title',
            title: 'MBBS in Kazakhstan'
        },

        {
            "type": 'text',
            "title": '',
            //"img": 'https://www.selectyouruniversity.com/images/kazakhstan/mbbs-in-kazakhstan.jpg',
            "data": '<p>Every year more than 10,000 Indian students get MBBS admission abroad. MBBS in Kazakhstan for Indian students is always worth money as medical education is available at very minimal rates.</p><p>Kazakhstan medical universities are globally admired because of their high-quality schooling teaching methodology. In Kazakhstan, Clinical clinical training is truly worthwhile since the nation continues the common benchmark of medical training.</p><p>All top medical universities in Kazakhstan are recognized by WHO, USMLE, IMED, GMC, NMC.</p><p>Kazakhstan progressed as a regional heart for Central Asia. MBBS in Kazakhstan is an immensely valued medical degree with the best medical universities.</p><p>Kazakhstan provides a diverse and excellent opportunity where Indian students can pursue medical education. Hence, Kazakhstan is chosen over other places and has evolved as a major educational hub for Indian students. To bag admission to a MBBS seat in the best medical universities in Kazakhstan is simple when compared to several other countries. </p>            '
        },

        {
            "type": "table-title",
            "title": "MBBS in Kazakhstan - Quick Highlights 2022",
            "info": "<b>Check the brief attributes of studying medical course in Kazakhstan:</b>",
            "col": [
            ],
            "row": [
                ["<b>Intake for MBBS Cours</b>", "October"],
                ["<b>Basic Eligibility</b>", "60% in PCB for General<br> 40% for SC/ST"],
                ["<b>NEET Exam</b>", "Yes, it is compulsory"],
                ["<b>IELTS & TOEFL</b>", "Not required"],
                ["<b>Minimum Course Fee</b>", "Rs. 2.3 Lakhs/Year"],
                ["<b>Maximum Course Fee</b>", "Rs. 5 Lakhs/Year"],
                ["<b>Top Medical Universities</b>", "Al-Farabi Kazakh National University,<br>Kazakh National Medical University"],
                ["<b>Cost of Living in Kazakhstan</b>", "Rs. 5 - 10 Thousands/Month"],
                ["<b>Course Duration</b>", "6 (including 1 year internship)"],
                ["<b>Medium of Teaching</b>", "English"],
                ["<b>Universities Recognition</b>", "NMC & WHO approved"],


            ]
        },
     {
            "type": "list-bollet",
            "title": "Why Should you Study Medicine in Kazakhstan?",
            "info": "<b>These are the reasons for pursuing MBBS course in the Universities of Kazakhstan:</b>",
            //"img": 'https://www.selectyouruniversity.com/images/benefits-of-mbbs-in-kazakhstan.jpg',
            "data": [
                "Affordable tuition fee structure.",
                "Best Teacher - Student Ratio",
                "The curriculum is designed as per the international standard.",
                "Globally recognized medical degree.",
                "Offer high quality education at cheaper fees.",
                "Listed in World Directory of Medical Colleges.                ",
                "No donation or capitation fees to get a MBBS seat in the best medical universities.",
                "No IELTS & TOEFL.",
                "After completing the MBBS course from Kazakhstan medical universities, students are eligible to continue their practice in any part of the world.",
                "The medical universities have modern technology and capable infrastructure.",
                "The Cost of living in Kazakhstan is very cheap as compared to other countries in Europe.",
                "Tuition fee and accommodation are affordable.",


            ]
        },
        {
            "type": "list-bollet",
            "title": "Eligibility Criteria",
            "info": "<b>Students taking admission in Kazakhstan must meet the following eligibility criteria:</b>",
            //"img": 'https://www.selectyouruniversity.com/images/kazakhstan/eligibility-criteria-of-mbbs-in-kazakhstan.jpg',
            "data": [
                "The student must have obtained a minimum of 50% marks in PCB (Physics, Chemistry & Biology) for general category & 40% for SC/ST and OBC as per NMC rule.",
                "The applicant must be or above 17 years of age.",
                "No entrance examination is required for admission.",
                "No upper age limit to study in Kazakhstan",


            ]
        },


        {
            type: 'step-list',
            title: 'Admission Procedure for Studying MBBS in Kazakhstan',
            info: '<b>Here is an overview of the admission process followed in Kazakhstan:</b>',
            //img: 'https://www.selectyouruniversity.com/images/kazakhstan/admission-process-to-study-mbbs-in-kazakhstan.jpg',
            data: [
                "<b>Step 1:</b> First, applicant should fill the application form with all the correct points of interest.",
                "<b>Step 2:</b> After universitys acceptance, you will receive the admission letter from us.",
                "<b>Step 3:</b> After admission letter, student has to submit his passport to our Delhi Office.",
                "<b>Step 4:</b> Then,we will apply for student visa in the Embassy of Kazakhstan in Delhi. Embassy may call the student for an interview.",
                "<b>Step 5:</b> Once visa is stamped we will notify the aspirant about the travelling date.",
                "<b>Step 6:</b> Once the student reaches Kazakhstan, university officials receive the student upon arrival.",
                "<b>Step 7:</b> Student will be accommodated to the university hostel. ",
                "<b>Step 8:</b> Student will pay the balance tuition fee to the international student department and joins the MBBS course."
            ]
          },
        {
            "type": "list-bollet",
            "title": "Required Document for MBBS Admission",
            "info": "<b>Below are the points of documents required to study MBBS course in Kazakhstan:</b>",
            //"img": 'https://www.selectyouruniversity.com/images/document-required-for-mbbs-admission-in-kazakhstan.jpg',
            "data": [
                "Photocopy of Passport",
                "5 passport size photos",
                "Birth certificate and 10th & 12th mark sheets",
                "Clearance Certificate of Police (PCC, Obtained after Receiving Passport)",
                "Bank Statement"
            ]
        },
        {
            "type": "list-bollet",
            "title": "MBBS in Kazakhstan - Is it Safe for Indian Students?",
            "info": "<b>Know why Kazakhstan is one of the safest countries for international studies:</b>",
            "img": '',
            "data": [
                "In a survey conducted by the Kazakhstan government - India ranked among the top 50 countries sending a large number of students to Kazakhstan.",
                "Тhе Іndіаn studеnts whо currently pursuing МВВЅ course іn Kazakhstan аrе рrоvіdеd adequate sесurіtу аnd sаfеtу by their government authorities.",


            ]
        },
        {
            type:"meta",
            meta: [
              { name: "title", content: "MBBS in Kazakhstan for Indian Students - Check Fees & Eligibility" },
              { name: "description", content: "Study MBBS in Kazakhstan with top NMC approved medical universities at low fees & direct admission for Indian students. Check now for more details" },
            ],
            },
            {
            type:"meta-title",
            title: "MBBS in Kazakhstan for Indian Students - Check Fees & Eligibility"},
            {
            type:"meta-links",
            link: [
              { rel: "canonical", href: "https://www.selectyouruniversity.com/course/mbbs-in-kazakhstan" }
            ]}

    ],
    "syllabus":[
        {
            "type": "text",
            "title": "Total Duration of MBBS in Kazakhstan",
            "info": "Time required to earn the MBBS degree in Kazakhstan:",
            "data":"<p>It takes <b>6 years</b> to complete the MBBS course in the universities of Kazakhstan. 1 year is reserved for the internship period at the end of the last semester."
        },
        {
            "type": "table",
            "title": "Syllabus of MBBS Course in Kazakhstan",
            "info": "Refer to the syllabus given below that is designed to prepare advanced subjects that students study in Kazakhstan:",
            "col": [
                "1st Year",
                "2nd Year",
            ],

            "row": [
                {
                    "1st Year": "Fundamentals of Psychology",
                    "2nd Year": "Human Anatomy",
                },
                {
                    "1st Year": "Bioorganic Chemistry",
                    "2nd Year": "Histology",
                },
                {
                    "1st Year": "Histology",
                    "2nd Year": "Biochemistry",
                },
                {
                    "1st Year": "Medical Chemistry",
                    "2nd Year": "Physiology",
                },
                {
                    "1st Year": "Embryology",
                    "2nd Year": "Philosophy",
                },
                {
                    "1st Year": "Human Anatomy",
                    "2nd Year": "Emergency Medicine",
                },
                {
                    "1st Year": "Cytology",
                    "2nd Year": "Microbiology",
                },
                {
                    "1st Year": "Elective Courses",
                    "2nd Year": "Immunology",
                },
                {
                    "1st Year": "-",
                    "2nd Year": "Patient Care",
                },
                {
                    "1st Year": "-",
                    "2nd Year": "Embryology",
                }
            ]
        },
        {
            "type": "table",
            "title": "",
            "info": "",
            "col": [
                "3rd Year",
                "4th Year",
            ],

            "row": [
                {
                    "3rd Year": "Pathophysiology",
                    "4th Year": "Pediatrics",
                },
                {
                    "3rd Year": "Pharmacology",
                    "4th Year": "Internal Medicine",
                },
                {
                    "3rd Year": "Pathomorphology",
                    "4th Year": "Tuberculosis",
                },
                {
                    "3rd Year": "Hygiene and Ecology",
                    "4th Year": "Surgery",
                },
                {
                    "3rd Year": "Pediatrics",
                    "4th Year": "Obstetrics and Gynecology",
                },
                {
                    "3rd Year": "Internal Medicine",
                    "4th Year": "Neurology",
                },
                {
                    "3rd Year": "General Surgery",
                    "4th Year": "Ophthalmology",
                },
                {
                    "3rd Year": "Radiology",
                    "4th Year": "Medical Psychology",
                },
                {
                    "3rd Year": "Elective courses",
                    "4th Year": "Urology",
                },
                {
                    "3rd Year": "-",
                    "4th Year": "Narcology",
                },
                {
                    "3rd Year": "-",
                    "4th Year": "Forensic Medicine",
                },
                {
                    "3rd Year": "-",
                    "4th Year": "Biostatistics",
                },
                {
                    "3rd Year": "-",
                    "4th Year": "Social medicine",
                }
            ]
        },
        {
            "type": "table",
            "title": "",
            "info": "",
            "col": [
                "5th Year",
                "6th Year",
            ],

            "row": [
                {
                    "5th Year": "Epidemiology",
                    "6th Year": "Internal Medicine",
                },
                {
                    "5th Year": "Oncology",
                    "6th Year": "Infectious Diseases",
                },
                {
                    "5th Year": "Social Medicine",
                    "6th Year": "Pediatrics",
                },
                {
                    "5th Year": "Internal Medicine",
                    "6th Year": "Surgery",
                },
                {
                    "5th Year": "Surgery",
                    "6th Year": "General Practice",
                },
                {
                    "5th Year": "Pediatrics",
                    "6th Year": "Social medicine",
                },
                {
                    "5th Year": "Medical genetics",
                    "6th Year": "Oncology",
                },
                {
                    "5th Year": "Traumatology",
                    "6th Year": "Obstetrics",
                },
                {
                    "5th Year": "Gynecology",
                    "6th Year": "Elective Courses",
                },
                {
                    "5th Year": "Clinical Immunology",
                    "6th Year": "-",
                },
                {
                    "5th Year": "Neurosurgery",
                    "6th Year": "-",
                },
                {
                    "5th Year": "Obstetrics",
                    "6th Year": "-",
                },
                {
                    "5th Year": "Emergency Medicine",
                    "6th Year": "-",
                },
                {
                    "5th Year": "Fundamentals of Stomatology",
                    "6th Year": "-",
                }
            ]
        },
        {
            "type": 'list-bollet',
            "title": 'Faculties for MBBS in Kazakhstan',
            "info": 'See the different types of medicine-related courses offered in Kazakhstan:',
            "data": [
                "General Medicine",
                "Therapeutic",
                "Pediatrics",
                "Medico-prophylactic",
                "Stomatology",
                "Pharmacy",
                "Public Health and Pharmacy",
                "Postgraduate"
            ]
        },
        {
            type:"meta",
            meta: [
              { name: "title", content: "MBBS in Kazakhstan 2023 - Syllabus" },
              { name: "description", content: "Have a look at the detailed syllabus for the course listed by the country. Check on the link provided  for detailed information" },
            ],
            },
            {
            type:"meta-title",
            title: "MBBS in Kazakhstan 2023 - Syllabus"},
            {
            type:"meta-links",
            link: [
              { rel: "canonical", href: "https://www.selectyouruniversity.com/course/mbbs-in-kazakhstan/syllabus" }
            ]}
    ],
        "topcolleges":[
            {
                "type": "table",
                "title": "MBBS in Kazakhstan Fee Structure 2023",
                "info": "Here are the average tuition fees and hostel fees of the top universities in Kazakhstan:",
                "col": [
                  "Name of the University",
                  "Tuition Fees (For 1 Year)",
                  "Hostel Fee (For 1 Year)"
                ],
                "row": [
                  {
                    "Name of the University":"<b>Kazakh National Medical University</b>",
                    "Tuition Fees (For 1 Year)":"4500 USD",
                    "Hostel Fee (For 1 Year)":"650 USD"
                  },
                  {
                    "Name of the University":"<b>Astana Medical University</b>",
                    "Tuition Fees (For 1 Year)":"3200 USD",
                    "Hostel Fee (For 1 Year)":"700 USD"
                  },
                  {
                    "Name of the University":"<b>UIB International Medical School</b>",
                    "Tuition Fees (For 1 Year)":"3500 USD",
                    "Hostel Fee (For 1 Year)":"700 USD"
                  },
                  {
                    "Name of the University":"<b>South Kazakhstan Medical Academy</b>",
                    "Tuition Fees (For 1 Year)":"4250 USD",
                    "Hostel Fee (For 1 Year)":"700 USD"
                  },
                  {
                    "Name of the University":"<b>Karaganda State Medical University</b>",
                    "Tuition Fees (For 1 Year)":"3700 USD",
                    "Hostel Fee (For 1 Year)":"600 USD"
                  },
                  {
                    "Name of the University":"<b>Kazakh Medical University of Continuing Education</b>",
                    "Tuition Fees (For 1 Year)":"3500 USD",
                    "Hostel Fee (For 1 Year)":"700 USD"
                  },
                  {
                    "Name of the University":"<b>Kazakh Russian Medical University</b>",
                    "Tuition Fees (For 1 Year)":"4000 USD",
                    "Hostel Fee (For 1 Year)":"700 USD"
                  },
                  {
                    "Name of the University":"<b>Kokshetau State University</b>",
                    "Tuition Fees (For 1 Year)":"3500 USD",
                    "Hostel Fee (For 1 Year)":"500 USD"
                  },
                  {
                    "Name of the University":"<b>North Kazakhstan State University</b>",
                    "Tuition Fees (For 1 Year)":"3500 USD",
                    "Hostel Fee (For 1 Year)":"500 USD"
                  },
                  {
                    "Name of the University":"<b>Semey State Medical University</b>",
                    "Tuition Fees (For 1 Year)":"3200 USD",
                    "Hostel Fee (For 1 Year)":"600 USD"
                  },
                  {
                    "Name of the University":"<b>West Kazakhstan Marat Ospanov State Medical University</b>",
                    "Tuition Fees (For 1 Year)":"3500 USD",
                    "Hostel Fee (For 1 Year)":"500 USD"
                  },
                  {
                    "Name of the University":"<b>Al-Farabi Kazakh National University</b>",
                    "Tuition Fees (For 1 Year)":"4200 USD",
                    "Hostel Fee (For 1 Year)":"700 USD"
                  }
                ],
                "heading":"<p>Tuition fees of the medical universities mentioned in the table are subject to fluctuation. Contact Select Your University for more details regarding tuition fees, hostel cost, and conversion of USD to INR.</p>"
            },
            {
                "type": "table",
                "title": "Top Medical Universities in Kazakhstan - Ranking 2023",
                "info": "National & International ranking of all the top medical colleges in Kazakhstan:",
                "col": [
                  "Name of the University",
                  "Country Ranking",
                  "World Ranking"
                ],
                "row": [
                  {
                    "Name of the University":"<b>Kazakh National Medical University</b>",
                    "Country Ranking":"6",
                    "World Ranking":"4279"
                  },
                  {
                    "Name of the University":"<b>Karaganda State Medical University</b>",
                    "Country Ranking":"34",
                    "World Ranking":"7540"
                  },
                  {
                    "Name of the University":"<b>Semey State Medical University</b>",
                    "Country Ranking":"15",
                    "World Ranking":"6008"
                  },
                  {
                    "Name of the University":"<b>South Kazakhstan State Medical Academy</b>",
                    "Country Ranking":"21",
                    "World Ranking":"6901"
                  },
                  {
                    "Name of the University":"<b>West Kazakhstan State Medical Academy</b>",
                    "Country Ranking":"54",
                    "World Ranking":"9192"
                  }
                ],
            },
        {
            type:"meta",
            meta: [
              { name: "title", content: "MBBS in Kazakhstan 2023 - Top Colleges in Kazakhstan"
             },
              { name: "description", content: "Get all the details about the top colleges present in the country for your desirable education. Click on the link and  get access to the Information" },
            ],
            },
            {
            type:"meta-title",
            title: "MBBS in Kazakhstan 2023 - Top Colleges in Kazakhstan"
        },
            {
            type:"meta-links",
            link: [
              { rel: "canonical", href: "https://www.selectyouruniversity.com/course/mbbs-in-kazakhstan/top-colleges" }
            ]}
    ],
    "careeroptions":[
        {
            "type": 'list-bollet',
            "title": 'Career Opportunities After Completing MBBS from Kazakhstan',
            "info": 'These are the career opportunities for MBBS graduates in Kazakhstan:',
            "data": [
                "Students can choose post graduation courses at any international university.",
                "Indian students can return to India to start their medical profession.",
                "Clearance of NEXT examination with good marks is mandatory to start a career in India."
            ]
        },
        {
            type:"meta",
            meta: [
              { name: "title", content: "MBBS in Kazakhstan 2023 - Career Options" },
              { name: "description", content: "See all the career options you can look upto after completing your education. Click at the link and have a look at all the options available" },
            ],
            },
            {
            type:"meta-title",
            title: "MBBS in Kazakhstan 2023 - Career Options"},
            {
            type:"meta-links",
            link: [
              { rel: "canonical", href: "https://www.selectyouruniversity.com/course/mbbs-in-kazakhstan/career-options" }
            ]}
    ],
    "faq": [
        {
            "type": "question-list",
            "title": "MBBS in Kazakhstan - Important FAQs",
            "info": "Apart from the information provided, here are some frequently asked questions on Quora and Careers 360:",
            "data": [
                {
                  "ques": "Q. How many Indian students are studying in Kazakhstan?",
                  "ans": "There are more than 8000 Indian students studying in Kazakhstan out of these, 5000 students are pursuing medical degree."
                },
                { "ques": "Q. Which are the highly recommended medical universities for Indian students?",
                  "ans": "Kazakh National Medical University and Semey State Medical University have great infrastructure and a track record of successful alumni. These universities are more preferable for Indian students to build the foundation of their career in medicine."
                },
                {
                    "ques": "Q. When does the MBBS course Start in Kazakhstan?",
                    "ans": "In October month the intake of the NMC approved medical universities of Kazakhstan starts. The admission forms for the same start from August-September."
                },
                {
                    "ques": "Q. Is Kazakhstan Good Option for MBBS?",
                    "ans": "Yes, it is. See, Kazakhstan has a couple of NMC approved medical universities that provide quality education to each foreign student. One more thing that attracts most of the Indian students is that medical universities in Kazakhstan offer a 6 year MD program at affordable fees."
                },
                {
                    "ques": "Q. Can I get the scholarships or any kind of subsidized allowances while studying MBBS in Kazakhstan?",
                    "ans": "There are some medical universities in Kazakhstan which offer scholarships, fellowships based on a merit basis or as per the university norms. & yes, for the foreign students partial funding is provided by the Government depending on the type of the universities."
                },
                {
                    "ques": "Q. How many NMC recognized medical colleges are there in Kazakhstan?",
                    "ans": "There are around 12 medical colleges in Kazakhstan recognized by the NMC."
                },
                {
                    "ques": "Q. Is there any language barrier while studying MBBS in Kazakhstan?",
                    "ans": "English is the most commonly taught language in Kazakhstan, especially for foreign students. Apart from this, Russian and Kazakh are the primary languages that people used to speak in Kazakhstan."
                },
                {
                    "ques": "Q. After MBBS from Kazakhstan, Is the NMC screening test required or not?",
                    "ans": "No, not required. You have to clear the NEXT exam i.e. (National Exit Test) at the end of your academic year. After completion of the MBBS course from Kazakhstan, you can practice anywhere in India or abroad."
                },
                {
                    "ques": "Q. Is MBBS degree from Kazakhstan valid in India?",
                    "ans": "Yes. MBBS degree from Kazakhstan accepted worldwide & recognized by the world medical councils."
                },
                {
                    "ques": "Q. What is the safety index of Kazakhstan for Indians?",
                    "ans": "It’s about 61% which is the huge percentile in terms of safety."
                }, 
            ]
        },
        {
            type:"meta",
            meta: [
              { name: "title", content: "MBBS in Kazakhstan 2023 - FAQs" },
              { name: "description", content: "Read all the FAQs asked by the students like you and clear all your doubts. Click here and read all the FAQs " },
            ],
            },
            {
            type:"meta-title",
            title: "MBBS in Kazakhstan 2023 - FAQs"},
            {
            type:"meta-links",
            link: [
              { rel: "canonical", href: "https://www.selectyouruniversity.com/course/mbbs-in-kazakhstan/faq" }
            ]}
    ]
}
