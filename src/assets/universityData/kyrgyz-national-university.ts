export const kyrgyznationaluniversity = {
    "Title": "Kyrgyz National University, Kyrgyzstan",
    "shortTitle": "Kyrgyz National University",
    "location": "  Κуrgуzstаn",
    "type": "bb",
    "rating": "vv",
    "shareLink": "www.google.com",
    "url": "kyrgyz-national-university",
    "info": [
        {
            type: 'title',
            title: 'Kyrgyz National University, Kyrgyzstan'
        },

        {
            type: 'text',
            title: 'About Kyrgyz National University, Kyrgyzstan',
            img: 'https://www.selectyouruniversity.com/images/kyrgyzstan-universities/kyrgyz-national-university.jpg',
            data: '<p>Named after <b>Jusup Balasagyn, The Kyrgyz National University</b> is a national college in Kyrgyzstan. It is one of the oldest and the biggest Higher Education establishments in Kyrgyzstan. The University is placed withinside the capital city of Bishkek. The institution was established on 25 October 1925 as the Kyrgyz Institute of Education.</p><p>Just Kyrgyz National University commenced its Faculty of Medicine in collaboration with <b>LN University, India</b> and <b>LN Medical College Kyrgyz National University</b> became the primary University in Central Asia to expand a Faculty of Medicine campus in partnership with an Indian University. Today the Kyrgyz National University gives a 5 year MD Physician application using the teaching platform of LN University, Bhopal, India</p><p>As the number of students enrolled is 15,000 -19,999 this shows quality which the university offers, also with 1,500 -1,999 Kyrgyz National University has a huge team of academic staff to support students in each way. As of the 2016 - 2017 instructional year, there have been 2,654 teaching teams of workers at university. Of them, 1,908 people or 71.9% of the overall number of full-time instructors held a scientific degree. The university additionally employs around 271 part-time or non-coaching team of workers. Of these employees, 183 (or 67.5%) additionally keep a top degree. Counting each full-time and part-time instructor, it hired 67 professors and 339 associate professors.</p><p>Kyrgyz National University offers MBBS courses at low tuition fees with the average sum of Rs 2 lacs and the average of the hostel being Rs 20K with the important entities required. The institute offers numerous academic and non-academic centers and services to college students together with a library, housing, sports activities facilities, in addition to administrative services. Also the medium of education at the University is English which is comfortable for everyone.</p>'
        },

        {
            "type": "table-title",
            "title": "Kyrgyz National University - Quick Overview 2022",
            "info": "Given below is the table for quick highlights about Jusup Balasagyn Kyrgyz National University have a look at the details carefully and know more about the university:",
            "col": [
            ],
            "row": [
                ["<b>Program Name</b>", "General Medicine"],
                ["<b>Degree offered</b>", "MD Physicians"],
                ["<b>Year of Establishment</b>", "1925"],
                ["<b>Duration of Study</b>", "5 years"],
                ["<b>Academic Session Starts</b>", "March and September"],
                ["<b>Language of Instruction</b>", "English"],
                ["<b>Type</b>", "State"],
                ["<b>Eligibility Criteria</b>", "50% aggregate in PCB(Physics, Chemistry and Biology)"],
                ["<b>Entrance Exam</b>", "Yes, Its Required"],
                ["<b>Annual Tuition Fees</b>", "4000 USD"],
                ["<b>Laboratories</b>", "12 well equipped laboratories"],
                ["<b>Library</b>", "Well equipped library with over 5000 books"],
                ["<b>Approved</b>", "The Ministry of Education and Science, Kyrgyzstan"],
                ["<b>University Ranking</b>", "Country Rank - 5<br>World Rank - 6781"],


            ]
        },
        {
            "type": "list-bollet",
            "title": "Why Study MBBS at Kyrgyz National University?",
            "info": "The advantages to studying at Kyrgyz National University give you some decent reasons to choose your MBBS degree from this institute in Bishkek:",
            "img": '',
            "data": [
                "Kyrgyz National University is located in Bishkek, having the lowest fee in government universities of Kyrgyzstan.",
                "Kyrgyz National University has more than 50+ global college students in MBBS & MD General Medicine that are from distinct nationalities Pakistan, India, Bangladesh, Nepal & others.",
                "Kyrgyz National University has a modular system that is a modern medical training system.",
                "Many Female college students are already studying MBBS in this institute in Secure environment.",
                "Kyrgyz National University Bishkek hostels are separate for females and male students with 3 instance mess facilities with a pretty affordable fee of 100 USD per month.",
                "MBBS’s Course Duration is 6 years along with 1 year of Clinical Internship or Clinical Rotation (4+1).",
                "The University has 4+ affiliated hospitals.",
                "The MD degree is authorized through PM&DC/PMC/NMC guidelines & degree is equal to Medical graduates of WHO.",
            ]
        },

        {
            "type": "list-bollet",
            "title": "Affiliated Hospitals",
            "info": "Kyrgyz National University has 4 affiliated hospitals which are listed below, have a look at it:",
            "img": '',
            "data": [
                "City Clinical Hospital,",
                "International Medical Center On Clinic",
                "National Hospital under the Ministry of Health of the Kyrgyz Republic",
                "City Children's Clinical Hospital.",


            ]
        },


        {
            "type": "table",
            "title": "Kyrgyz National University - Ranking 2022",
            "info": "According to unipage.net, the country & world ranking of Kyrgyz National University is as follows:",
            "col": [
                "World",
                "Country",
            ],

            "row": [
                {
                    "World": "5",
                    "Country": "6781",
                },

            ]
        },



        {
            type: 'text',
            title: 'Medium of Teaching',
            info: 'Following is medium of teaching at Kyrgyz National University:</b>',
            data: '<p>At this university, all the different medicine courses are taught in the English language.</p> '
        },
        {
            "type": "list-bollet",
            "title": "Accreditation and Recognition",
            "info": "The Kyrgyz National University has international recognition and accreditation. It is recognized by:",
            "img": '',
            "data": [
                "The Ministry of Education and Science, Kyrgyzstan",
                "WHO [ World Health Organization ]",
                "NMC [ National Medical Commision ]",


            ]
        },


        {
            "type": "list-bollet",
            "title": "Library at Kyrgyz National University",
            "info": "Library of Kyrgyz National University is considered as one of the biggest library in Kyrgyzstan, the library contains 5000+ books which may help you gain immense knowledge have look at the below section for details:",
            "img": '',
            "data": [
                "The Kyrgyz National University Scientific Library is certainly considered one among the biggest college libraries in Kyrgyzstan.",
                "The clinical library was established in 1932 on the idea of the Kyrgyz Pedagogical Institute.",
                "In 1941, the library's book stock became 134,000 texts, many acquired from the colleges of Moscow, Leningrad and different main cities of the USSR.",
                "In 1951 the library acquired educational, medical and artistic literature from universities in Odessa, Kiev, Kharkov, Kazakh and Tomsk, in addition to from the Academies of Sciences of Armenia, Georgia, Uzbekistan and Ukraine. Today, the library is placed in 8 educational homes and has 60 employees.",


            ]
        },
        {
            "type": "list-bollet",
            "title": "Facilities Provided by Kyrgyz National University",
            "info": "Kyrgyz National University has a wide range of facilities to its students which can help you with studies as well as refreshment :",
            // "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/library.png",
            "data": [
                "Library",
                "Canteen",
                "hostel",
                "Play Ground",
                "Gym",
                "Sports",
                "ATM",
                "Hospital",
                "Auditorium",
                "Computer labs",
                "Laboratory",
            ]
        },
        // "<p class =text-center>Library</p>"

        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/canteen.png",
        //     "data": [
        //         "<p class =text-center>Canteen</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/hostel.png",
        //     "data": [
        //         "<p class =text-center>Hostel</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/play-ground.png",
        //     "data": [
        //         "<p class =text-center>Play Ground</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/gym.png",
        //     "data": [
        //         "<p class =text-center>Gym</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/sport.png",
        //     "data": [
        //         "<p class =text-center>Sport</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/atm.png",
        //     "data": [
        //         "<p class =text-center>ATM</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/hospital.png",
        //     "data": [
        //         "<p class =text-center>Hospital</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/auditorium.png",
        //     "data": [
        //         "<p class =text-center>Auditorium</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/computer-labs.png",
        //     "data": [
        //         "<p class =text-center>Computer labs</p>"
        //     ]
        // },
        // {
        //     "type": "text",
        //     "title": "",
        //     "img": "https://www.selectyouruniversity.com/images/kyrgyzstan-universities/laboratory.png",
        //     "data": [
        //         "<p class =text-center>Laboratory</p>"
        //     ]
        // },
        {
            "type": "table",
            "title": "Monthly Living Expenses in Bishkek",
            "info": "<b>Bishkek city has low Living cost which is affordable for global medical aspirants, and the monthly based expenses of Bishkek are given below in the following table:</b>",
            "col": [
                "Particulars",
                "Approx Range in USD",
                "Approx Range in INR",
            ],

            "row": [
                {
                    "Particulars": "<b>Food</b>",
                    "Approx Range in USD)": "150 USD",
                    "Approx Range in INR": "11,250 INR",
                },
                {
                    "Particulars": "<b>Transportation</b>",
                    "Approx Range in USD)": "6 USD",
                    "Approx Range in INR": "468 INR",
                },
                {
                    "Particulars": "<b>Other Expenses (milk, bread, eggs, clothing)</b>",
                    "Approx Range in USD)": "23 USD",
                    "Approx Range in INR": "1,725 INR",
                },
            ],
            "heading": "<p><b>Note:</b> $1 = Rs.75 (for calculation purpose.)</p>"
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Fees, Courses, Ranking" },
                { name: "description", content: "Kyrgyz National University is a 97 years old university in Kyrgyzstan. Established in 1925, it offers cost-efficient option to study MBBS in Kyrgyzstan" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Fees, Courses, Ranking"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university" }
            ]
        }
    ],
    "coursesandfees": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - courses & Fees"

        },
        {
            "type": "table",
            "title": "Kyrgyz National University - MBBS Fee Structure 2022",
            "info": "MBBS course at Kyrgyz National Medical University is in affordable price with of an average sum per year be Rs 2 lacs and average sum of hostel be Rs 20 thousand for detailed info have a look below:",
            "col": [
                "Particular",
                "Year 1",
                "Year 2",
                "Year 3",
                "Year 4",
                "Year 5",
                "Year 6"

            ],
            "row": [
                {
                    "Particular": "<b>Tuition Fee</b>",
                    "Year 1": "$ 4000",
                    "Year 2": "$ 4000",
                    "Year 3": "$ 4000",
                    "Year 4": "$ 4000",
                    "Year 5": "$ 4000",
                    "Year 6": "$ 4000"
                },
                {
                    "Particular": "<b>Hostel Fee</b>",
                    "Year 1": "$ 1800",
                    "Year 2": "$ 1800",
                    "Year 3": "$ 1800",
                    "Year 4": "$ 1800",
                    "Year 5": "$ 1800",
                    "Year 6": "$ 1800"

                },
                {
                    "Particular": "<b>Total Fees in USD</b>",
                    "Year 1": "<b>$ 5800</b>",
                    "Year 2": "<b>$ 5800</b>",
                    "Year 3": "<b>$ 5800</b>",
                    "Year 4": "<b>$ 5800</b>",
                    "Year 5": "<b>$ 5800</b>",
                    "Year 6": "<b>$ 5800</b>"

                },
                {
                    "Particular": "<b>Total Fees in Rs</b>",
                    "Year 1": "<b>Rs. 4,11,800</b>",
                    "Year 2": "<b>Rs. 4,11,800</b>",
                    "Year 3": "<b>Rs. 4,11,800</b>",
                    "Year 4": "<b>Rs. 4,11,800</b>",
                    "Year 5": "<b>Rs. 4,11,800</b>",
                    "Year 6": "<b>Rs. 4,11,800</b>"

                }
            ],
            "heading": "<p><b>Note: </b>$1 = Rs. 71 (or calculation purpose.)</p>",

        },
        {
            "type": "text",
            "title": "Kyrgyz National University - MBBS Syllabus (Semester Wise)",
            "info": 'The complete Year wise syllabus for studying MBBS at Kyrgyz National University have look carefully and prepare well:'
        },
        {
            "type": "table",
            "title": "",
            "info": "1st Year",
            "col": [
                "1st Semester",
                "2nd Semester"

            ],
            "row": [
                {
                    "1st Semester": "Anatomy",
                    "2nd Semester": "Histology"
                },
                {
                    "1st Semester": "-",
                    "2nd Semester": "Anatomy"
                },
            ]
        },
        {
            "type": "table",
            "title": "",
            "info": "2nd Year",
            "col": [
                "3rd Semester",
                "4th Semester"

            ],
            "row": [
                {
                    "3rd Semester": "Anatomy",
                    "4th Semester": "Bio-Chemistry"
                },
                {
                    "3rd Semester": "Anatomy",
                    "4th Semester": "Physiology"
                },
                {
                    "3rd Semester": "Bio-Chemistry",
                    "4th Semester": "Micro-Biology"
                },
                {
                    "3rd Semester": "Physiolog",
                    "4th Semester": "-"
                },
            ]
        },
        {
            "type": "table",
            "title": "",
            "info": "3rd Year",
            "col": [
                "5th Semester",
                "6th Semester"

            ],
            "row": [
                {
                    "5th Semester": "Pathology",
                    "6th Semester": "Path physiology"
                },
                {
                    "5th Semester": "Micro-Biology",
                    "6th Semester": "Pathology"
                },
                {
                    "5th Semester": "Pharmacology",
                    "6th Semester": "Pharmacology"
                },
                {
                    "5th Semester": "Path physiology",
                    "6th Semester": "-"
                },
            ]
        },
        {
            "type": "table-title",
            "title": "",
            "info": "4th - 6th Year",
            "colTitle": "7th-12th Semester",
            "row": [
                ["Clinical Subjects", "Clinical Postings"],
                ["Oncology", "Internal medicine"],
                ["Neurology", "Psychology"],
                ["Pediatrics", "General Surgery"],
                ["Gynecology", "ENT"],
                ["Emergency Medicine", "Cardiology"]
            ]
        },
        {
            "type": "list-bollet",
            "title": "Total MBBS Duration at Kyrgyz National University",
            "info": "Kyrgyz National University offers 5 years of medicine program with + 1 year of internship:",
            "data": [
                "MBBS’s Course Duration is 6 years which include 1 year of Clinical Internship or Clinical Rotation (4+1).",
                "The college students will go through theoretical & practical information all through the first 5 years and then 1 year of compulsory Internship for practical knowledge.",
            ]
        },
        {
            "type": "table-title",
            "title": "Departments at Kyrgyz National University",
            "info": "Kyrgyz National University has 25 various Departments in the premises, each department poses different knowledge, have a look at the list mentioned below:",
            "row": [
                ["Anatomy", "Physiology", "Biochemistry", "Social Medicine"],
                ["Pharmacology", "Forensic Medicine", "Pathology", "Microbiology"],
                ["Medicine", "Surgery", "Gynae & Obs.", "Pediatrics"],
                ["Pediatric Surgery", "Orthopedic Surgery", "ENT", "Anesthesiology"],
                ["Radiology and Imaging", "Cardiology", "Dermatology", "Ophthalmology"],
                ["Psychiatry", "Urology", "Oncologyy", "Transfusion Medicine"],
                ["Dental Department",],
            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - courses & Fees" },
                { name: "description", content: "Kyrgyz National University is a 97 years old university in Kyrgyzstan  ; Tuition Fee with Hostel. Check available courses and fees" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - courses & Fees"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/courses-and-fees" }
            ]
        }
    ],
    "admission": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Admission 2023"

        },
        {
            "type": "list-bollet",
            "title": "Kyrgyz National University - Eligibility Criteria",
            "info": "If any student desires to study MBBS in Jusup Balasagyn Kyrgyz National University then they must have to satisfy the following admission requirements:",
            "data": [
                "It would be great to be at the least 17 years old on or before 31st December of the admission year.",
                "Qualification Candidates need to have cleared 10+2 with 50% marks from the Science stream (Physics, Chemistry and Biology) in CBSE/ISC or another Equivalent Board of Examination.",
                "Minimum Qualifying (%): Unreserved: 50% & Reserved: 40%",
                "Qualifying Entrance: NEET Required",
            ]
        },
        {
            "type": "step-list",
            "title": "How to take Admission in Kyrgyz National University?",
            "info": "There will be a round of counseling and academic progress, the Admission process for Kyrgyz National University is based. Students are advised to read the below mentioned points to avoid any kind of confusion:",
            "data": [
                "<b>Step 1: Registration</b><br>Admission letter process: (Charges = 100$)<br>We will offer a legitimate receipt of registration fee to the student. Kyrgyz National University issues the admission letter in usually 3-7 operating days. Students will publish the following documents to us for admission letters.<br>Fresh photo with white background<br>SSC/O Levels Document<br>HSSC/Intermediate or A Levels Document<br>CNIC or Form B",
                "<b>Step 2: Visa Letter</b><br>Visa Invitation Letter Process: (Charges = 200$)<br>We will offer legitimate receipt of visa letter fee to the student. Visa invitation letter issues through Kyrgyz Foreign Ministry which could take 20-30 days usually (or depends). Students will have to submit the following documents for a Visa letter.<br>Passport First page Scan<br>Fresh photo with white background<br>SSC/O Levels Document (attested by foreign office)<br>HSSC/Intermediate or A Levels Document (attested by foreign office)<br>CNIC or Form B<br>Police Clearance Report (attested by foreign office)<br>Medical Check up Report by DHQ Hospital (attested by foreign office)",
                "<b>Step 3: Arrival</b><br>After issuance of the Visa invitation letter, students should buy the one way air ticket from their hometown to Bishkek (Manas Airport). We can assist our student to buy the ticket (if asked through the student). Students pay on arrival visa fees themselves. As the student will arrive at Bishkek airport we can offer airport Pickup.",
                "<b>Step 4: Pay University Fee</b><br>Students pay the committed rate on arrival in USD. Nothing will be charged extra. Fees once paid will now no longer be refundable. In case of refund, Students will follow the fee refund policy consistent with the rules and guidelines of the university.",

            ]
        },
        {
            "type": "list-bollet",
            "title": "Documents Required - For Applying MBBS Course",
            "info": "Before taking admission to Kyrgyz National University keep in mind to hold all of the documents with you, all the details are mentioned below:",
            "data": [
                "Passport (Minimum 18 months validity).",
                "Birth Certificate, 10th Certificate & Mark sheet.",
                "12th Certificate & Mark sheet.",
                "NEET scorecard",
                "10 passport size Photographs",
                "Official Invitation letter from the Medical University of Kyrgyzstan.",
                "Authorization of all documents from the Ministry of External Affairs, New Delhi.",
                "Legalization of all documents from the Kyrgyzstan Embassy.",
                "Visa fees.",
                "Bank receipt of University 1st year Tuition fee (required for some Universities).",
                "HIV test documents.",
            ]
        },
        {
            "type": "table",
            "title": "Kyrgyz National University - Admission Dates 2022",
            "info": "Every candidate should check the important dates before taking MBBS admission at Kyrgyz National University:",
            "col": [
                "Events",
                "Date",

            ],
            "row": [
                {
                    "Events": "<b>Application Start Date</b>",
                    "Date": "January"
                },
                {
                    "Events": "<b>Invitation Letter</b>",
                    "Date": "Within 2 weeks"
                },
                {
                    "Events": "<b>Admission Start Date</b>",
                    "Date": "- Fall intake (September-October)<br>- Spring intake (January-February)"
                },
            ]
        },
        {
            "type": "table-title",
            "title": "Faculties offered at Kyrgyz National University",
            "info": "There are multiple faculties present at the Kyrgyz National University, some of them maybe useful for you have look carefully:",
            "row": [
                ["State and Municipal Management.", "Mathematics, Computer Science and Cybernetics", "Physics and Electronics", "Information and Innovation Technologies"],
                ["Chemistry and Chemical Technology", "Biology", "Geography and Ecology and Tourism", "Kyrgyz Philology"],
                ["Russian and Slavic philology", "History and Regional Studies", "Journalism", "Social and Human Sciences"],
                ["Foreign Languages", "European Faculty", "International Relations and Oriental Studies", "Chinese Faculty"],
                ["Pedagogical Faculty", "Retraining and staff development", "State and Municipal Management.", "Economics"],
                ["Faculty of Law", "Management and business",],

            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Admission 2023" },
                { name: "description", content: "Admission step by step. Choose the appropriate section in the system of Internet Recruitment of Candidates.Click Here TO REGISTER" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Admission 2023"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/admission" }
            ]
        }
    ],
    "review": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Reviews"

        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Reviews" },
                { name: "description", content: "Find out what makes this college so special by reading student reviews. Click here to see reviews" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Reviews"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/reviews" }
            ]
        }
    ],
    "placement": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Placement"

        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Placement" },
                { name: "description", content: "Placement in the college is best because it has a record of 100% placement, Check top recruiters " },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Placement"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/placement" }
            ]
        }
    ],
    "gallery": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Gallery"

        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Gallery" },
                { name: "description", content: "See what life is like as a student at one of the best institutes. Check gallery here" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Gallery"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/gallery" }
            ]
        }
    ],
    "scholarship": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Scholarship"

        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Scholarship" },
                { name: "description", content: "college has a partnership with Siemens they provide scholarships who come from lower economical backgrounds. Check scholarship and bursary" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Scholarship"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/scholarship" }
            ]
        }
    ],
    "faculty": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Faculty"

        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Faculty" },
                { name: "description", content: "Browse list of faculties offered from this University to choose from. Click here for departments" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Faculty"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/faculty" }
            ]
        }
    ],
    "news": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - News & Articles"

        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - News & Articles" },
                { name: "description", content: "Read latest News, Articles about Cutoffs, Placements, courses, Fees. Check all the news and articles here" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - News & Articles"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/news-and-articles" }
            ]
        }
    ],
    "hostel": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - Hostel"

        },
        {
            "type": "text",
            "title": "Hostel and Accommodation at Kyrgyz National University",
            "info": "At Kyrgyz National University there luxurious rooms with all the needed amenities so here is all you need to know about hostel and accommodation:",
            "data": "<b>Amber</b><p>Amber Student takes the hassle out of securing student accommodation through supporting you to find the precise accomodation for your desires across the world, based on lots of exceptional accommodation. It enables withinside the reserving and paperwork, in addition to helping you on your search for roommates.</p><b>Unilodgers</b><p>Unilodgers is all about unlocking spaces in student-friendly neighborhoods around the globe and developing an international student community that is clearly your own. As the world’s biggest student housing provider, they create areas for college students to tackle the world and win.</p>",
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - Hostel" },
                { name: "description", content: "This college provides an excellent Hostel and Accommodation facilities, students can take advantage of it. Check detailed accomodation" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - Hostel"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/hostel" }
            ]
        }
    ],
    "faq": [
        {
            type: 'title',
            title: "Kyrgyz National University, Kyrgyzstan - FAQ"

        },
        {
            "type": "question-list",
            "title": "Important FAQs Related to Kyrgyz National University",
            "info": "",
            "data": [
                {
                    "ques": "Q. Is Jusup Balasagyn Kyrgyz National University NMC approved ?",
                    "ans": "Yes, the University is NMC & WHO recognized."
                },
                {
                    "ques": "Q. Is Kyrgyzstan an excellent place to study?",
                    "ans": "Kyrgyzstan has usually been one of the top preferred destinations for international college students to pursue their Medical Degree Courses."
                },
                {
                    "ques": "Q. What is the fee of living in Bishkek for studying MBBS at Jusup Balasagyn Kyrgyz State University?",
                    "ans": "Ranging around Rs.300 USD is the cost of living in Bishkek city. This may also range for every pupil according to their living style."
                },
                {
                    "ques": "Q. Does Bishkek have snow?",
                    "ans": "The snowy period lasts for 5.4 months, from October 23 to April 4, with a sliding 31-day snowstorm of at the least 1.0 inches."
                },
                {
                    "ques": "Q. Is Kyrgyzstan cold or warm?",
                    "ans": "In Kyrgyzstan, a mountainous country in Central Asia, the weather is continental, with cold winters, frequently frosty, and heat and sunny summers, once in a while scorchingly warm at low altitudes however more comfortable withinside the mountains."
                },
            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Kyrgyz National University, Kyrgyzstan - FAQ" },
                { name: "description", content: "the college is present among top university present in the country. Click here to see FAQs" },
            ],
        },
        {
            type: "meta-title",
            title: "Kyrgyz National University, Kyrgyzstan - FAQ"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/kyrgyz-national-university/faq" }
            ]
        }
    ]


}