export const andijanstatemedicalinstitute = {
    "Title": "Andijan State Medical Institute, Uzbekistan",
    "shortTitle": "",
    "location": "Andijan, Uzbekistan",
    "type": "",
    "rating": "",
    "shareLink": "",
    "url": "andijan-state-medical-institute",
    "info": [
        {
            type: 'title',
            title: 'Andijan State Medical Institute, Uzbekistan'
        },
        {
            "type": "text",
            "title": "",
            "data": "<p><b>Andijan State Medical Institute</b> was founded in 1955. It is located in one of the ancient cities of Fergana valley, ‘Andijan’ city. The Andijan State Medical Institute keeps the leading positions amongst higher education facilities of the Republic.</p><p>Andijan State Medical Institute has all the necessary prerequisites for research work and training process on a high level. The best part about Andijan State Medical Institute is that it is known to offer students education in 56 multiple departments. For students looking for MBBS in Uzbekistan, this institute is the right option.</p>"
        },
        {
            "type": "table-title",
            "title": "Andijan State Medical Institute - Quick Highlights 2023",
            "info": "Given below is the table for quick highlights about Andijan State Medical Institute:",
            "col": [

            ],
            "row": [
                ["<b>Intake for MBBS Course</b>", "September"],
                ["<b>Processing Time for MBBS Admission</b>", "25-30 days"],
                ["<b>Location</b>", "Andijan, Uzbekistan"],
                ["<b>Basic Eligibility</b>", "70% in PCB for General"],
                ["<b>NEET Exam</b>", "Yes, it is compulsory"],
                ["<b>Annual Tuition Fees (Approx.)</b>", "3400 USD"],
                ["<b>Course Duration</b>", "6 years (including Internship)"],
                ["<b>Medium of Education</b>", "English"],
                ["<b>University Ranking</b>", "Country - 15<br>World - 7896",],
                ["<b>University Recognition</b>", "Approved by NMC, WHO"],
            ]
        },
        {
            "type": "list-bollet",
            "title": "Why Study MBBS at Andijan State Medical Institute?",
            "info": "Below are some of the benefits of studying MBBS at Andijan State Medical Institute:",
            "data": [
                "Study programs at Andijan State Medical Institute include practical training along with theoretical knowledge which allows students to understand the concepts.",
                "All the medical programs at this university are available in English medium, which allows the students to pick up the concepts easily.",
                "The institution has many international and local tie-ups with other universities that allow open participation in student exchange programs, scientific research, etc. which helps the students to expand their horizons.",
                "At this university, the students have the freedom to choose from a wide range of specializations.",
                "Andijan State Medical Institute has a huge library with over 286000 books. In addition to this, there are 5 reading halls. The electronic library of the university has 40 CDs with the newest collection of articles on medicine.",
                "It has 700 bedded associated hospital that is equipped with modern diagnostic and treating instrumentations such as Scanners, endoscopes, MRT, MSCT, Scanning USI, Laparoscopic Center and others."
            ]
        },
        {
            "type": "table",
            "title": "Andijan State Medical Institute - Ranking 2023",
            "info": "According to 4icu.org, following is the country and world ranking for Andijan State Medical Institute:",
            "col": [
                "Country",
                "World"
            ],
            "row": [
                {
                    "Country": "15",
                    "World": "7896"
                }
            ]
        },
        {
            "type": "list-bollet",
            "title": "Accreditation and Recognition",
            "info": "Andijan State Medical Institute is accredited by many medical bodies such as:",
            "data": [
                "WHO (World Health Organization)",
                "NMC (National Medical Commission)",
                "And the Ministry of Higher and Secondary Specialized Education, Uzbekistan"
            ]
        },
        {
            "type": "table",
            "title": "Cost of Living at Andijan",
            "info": "The cost of living at Andijan city is as follows:",
            "col": [
                "Particulars",
                "Cost"
            ],
            "row": [
                {
                    "Particulars": "<b>Things of daily need (Milk, Groceries, Bread, etc.)</b>",
                    "Cost": "Ranges in between Rs.12 - Rs.325"
                },
                {
                    "Particulars": "<b>Eating at a Mid-Range Restaurant (for 2 people)</b>",
                    "Cost": "Rs.731 - Rs.750"
                },
                {
                    "Particulars": "<b>One Way Ticket of any Local Transport</b>",
                    "Cost": "Rs.18"
                },
                {
                    "Particulars": "<b>A monthly pass for local transport</b>",
                    "Cost": "RRs.993"
                },
                {
                    "Particulars": "<b>Monthly Rent for a 1 Bedroom Apartment</b>",
                    "Cost": "cost between Rs.10,000 - Rs.12,000"
                },
            ],
        },
        {
            "type": "list-bollet",
            "title": "About Andijan City",
            "data": [
                "Andijan is the Fergana Valley’s largest city having a population of 350,000 people.",
                "Both linguistically and culturally Andijon is probably the country’s purest Uzbek city.",
                "Andijan is the largest city of Uzbekistan, and it is also the largest center for the engineering industry.",
                "Andijan city is located in the southeastern part of Fergana Valley.",
                "The capital of Andijan region is the city of Andijan. The climate of Andijan is a typically continental climate with extreme differences between winter and summer temperatures.",
                "The average annual temperature of the city is 13.7 °C."
            ]
        },
        {
            type: 'meta',
            meta: [
                { name: "title", content: "Andijan State Medical Institute" },
                { name: "description", content: "Andijan State Medical Institute was established in 1955. It offers the best and affordable MBBS education in Uzbekistan. It ranks 57 in Uzbekistan." },
            ],
        },
        {
            type: 'meta-title',
            title: " Andijan State Medical Institute"
        },
        {
            type: 'meta-links',
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute" }
            ]
        }
    ],
    "coursesandfees": [
        {
            type: 'title',
            title: "Andijan State Medical Institute - Duration, Fees & Courses"
        },
        {
            "type": "table",
            "title": "Andijan State Medical Institute - MBBS Fee Structure 2023",
            "info": "The fee structure for Andijan State Medical Institute for MBBS program is given in the table below:",
            "col": [
                "Particular",
                "Year 1",
                "Year 2",
                "Year 3",
                "Year 4",
                "Year 5",
                "Year 6",

            ],
            "row": [
                {
                    "Particular": "<b>Tuition Fee</b>",
                    "Year 1": "3400 USD",
                    "Year 2": "3400 USD",
                    "Year 3": "3400 USD",
                    "Year 4": "3400 USD",
                    "Year 5": "3400 USD",
                    "Year 6": "3400 USD"
                },
                {
                    "Particular": "<b>Hostel Fee</b>",
                    "Year 1": "500 USD",
                    "Year 2": "500 USD",
                    "Year 3": "500 USD",
                    "Year 4": "500 USD",
                    "Year 5": "500 USD",
                    "Year 6": "500 USD"
                },
                {
                    "Particular": "<b>Total Fees in USD</b>",
                    "Year 1": "3900 USD",
                    "Year 2": "3900 USD",
                    "Year 3": "3900 USD",
                    "Year 4": "3900 USD",
                    "Year 5": "3900 USD",
                    "Year 6": "3900 USD"
                },
                {
                    "Particular": "<b>Total Fees in Rs</b>",
                    "Year 1": "<b>Rs. 3,12,000</b>",
                    "Year 2": "<b>Rs. 3,12,000</b>",
                    "Year 3": "<b>Rs. 3,12,000</b>",
                    "Year 4": "<b>Rs. 3,12,000</b>",
                    "Year 5": "<b>Rs. 3,12,000</b>",
                    "Year 6": "<b>Rs. 3,12,000</b>",

                },
            ],
            "heading": "<p><b>Note: </b>1 USD = Rs. 80 (It may Vary)</p>"
        },
        {
            "type": "list-bollet",
            "title": "Courses Offered at Andijan State Medical Institute",
            "info": "Andijan State Medical Institute has following faculties:",
            "data": [
                "Pedagogy",
                "Dentistry",
                "Postgraduate Program",
                "Gynecology",
                "Surgery",
                "Neuroscience",
                "Pediatrics",
            ]
        },
        {
            "type": "list-bollet",
            "title": "Andijan State Medical Institute - MBBS Duration",
            "info": "The duration of MBBS course at Andijan State Medical Institute is mentioned below:",
            "data": [
                "The duration of MBBS course at Andijan State Medical Institute is 6 years, out of which 1 year is internship.",
                "The medium of instruction is English for studying MBBS at this university.",

            ]
        },
        {
            "type": "table",
            "title": "Andijan State Medical Institute - MBBS Course Syllabus",
            "info": "The complete syllabus for studying MBBS at Andijan State Medical Institute is as follows:",
            "col": [
                "1st Year",
                "2nd Year",
            ],
            "row": [
                {
                    "First Year": "Medical Chemistry",
                    "Second Year": "Histology",
                },
                {
                    "First Year": "Fundamentals of Psychology",
                    "Second Year": "Physiology",
                },
                {
                    "First Year": "Histology",
                    "Second Year": "Human Anatomy",
                },
                {
                    "First Year": "Bioorganic Chemistry",
                    "Second Year": "Biochemistry",
                },
                {
                    "First Year": "Cytology",
                    "Second Year": "Microbiology",
                },
                {
                    "First Year": "Embryology",
                    "Second Year": "Emergency Medicine",
                },
                {
                    "First Year": "Human Anatomy",
                    "Second Year": "Patient Care",
                },
                {
                    "First Year": "Elective Courses",
                    "Second Year": "Embryology",
                },
            ]
        },
        {
            "type": "table",
            "title": "",
            "info": "",
            "col": [
                "3rd Year",
                "4th Year",
            ],
            "row": [
                {
                    "Third Year": "Pharmacology",
                    "Fourth Year": "Tuberculosis",
                },
                {
                    "Third Year": "Pathomorphology",
                    "Fourth Year": "Surgery",
                },
                {
                    "Third Year": "Pediatrics",
                    "Fourth Year": "Obstetrics and Gynecology",
                },
                {
                    "Third Year": "Internal Medicine",
                    "Fourth Year": "Urology",
                },
                {
                    "Third Year": "General Surgery",
                    "Fourth Year": "Neurology",
                },
                {
                    "Third Year": "Ecology",
                    "Fourth Year": "Ophthalmology",
                },
                {
                    "Third Year": "Radiology",
                    "Fourth Year": "Narcology",
                },
                {
                    "Third Year": "Elective Courses",
                    "Fourth Year": "Medical Psychology",
                },
                {
                    "Third Year": "Biostatistics",
                    "Fourth Year": "Forensic Medicine",
                },
                {
                    "Third Year": "-",
                    "Fourth Year": "Social Medicine",
                },
            ]
        },
        {
            "type": "table",
            "title": "",
            "info": "",
            "col": [
                "5th Year",
                "6th Year",
            ],
            "row": [
                {
                    "Fifth Year": "Infectious Diseases",
                    "Sixth Year": "General Practice",
                },
                {
                    "Fifth Year": "Oncology",
                    "Sixth Year": "Elective courses",
                },
                {
                    "Fifth Year": "Epidemiology",
                    "Sixth Year": "-",
                },
                {
                    "Fifth Year": "Gynecology",
                    "Sixth Year": "-",
                },
                {
                    "Fifth Year": "Traumatology",
                    "Sixth Year": "-",
                },
                {
                    "Fifth Year": "Medical Genetics",
                    "Sixth Year": "-",
                },
                {
                    "Fifth Year": "Social Medicine",
                    "Sixth Year": "-",
                },
                {
                    "Fifth Year": "Obstetrics",
                    "Sixth Year": "-",
                },
                {
                    "Fifth Year": "Clinical Immunology",
                    "Sixth Year": "-",
                },

            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute - Duration, Fees & Courses 2023" },
                { name: "description", content: "" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute - Duration, Fees & Courses 2023"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/courses-and-fees" }
            ]
        }
    ],
    "admission": [
        {
            type: 'title',
            title: "Andijan State Medical Institute - MBBS Admission, Eligibility Criteria"
        },
        {
            "type": "list-bollet",
            "title": "Eligibility Criteria",
            "info": "The eligibility criteria to study MBBS at Andijan State Medical Institute is as follows:",
            "data": [
                "Students must have cleared NEET exam.",
                "The student must have completed 17 years of age on 31st December of the year in which the admission is being taken.",
                "The candidate must have completed class 12th with at least 70% from a regular board.",
                "The student must have Physics, Chemistry and Biology at his/her 12th standard.",
                "A student must have valid identity proof and must have good command over English language.",
            ]
        },
        {
            "type": "step-list",
            "title": "Admission Process",
            "info": "Following is the admission process for applying to MBBS course at Andijan State Medical Institute:",
            "data": [
                "<b>Step 1:</b> At the beginning, a student needs to scan all the documents.",
                "<b>Step 2:</b> Now he/she must look forward to the application form available on the official website.",
                "<b>Step 3:</b> Then, he/she should fill the application form and submit.",
                "<b>Step 4:</b> After three to four days, they will get an offer letter after verification of documents.",
                "<b>Step 5:</b> Once students get the offer letter, they are supposed to pay the fees.",
                "<b>Step 6:</b> After paying the fees, students can apply for a visa. After getting the visa, they are ready to travel for MBBS in Uzbekistan.",
            ]
        },
        {
            "type": "list-bollet",
            "title": "Required Documents",
            "info": "Following documents are required at the time of admission to Andijan State Medical Institute:",
            "data": [
                "Birth certificate",
                "Passport copy (validity minimum 2 years)",
                "10th and 12th standard passing certificates",
                "Scorecard of NEET-UG examination qualified",
                "12 passport size photographs",
                "Character certificate",
                "Migration certificate",
                "Transfer certificate",
                "Proof of fee payment",
                "Financial proof (sufficient balance in the bank)",
            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute - MBBS Admission, Eligibility Criteria" },
                { name: "description", content: "Admission step by step. Choose the appropriate section in the system of Internet Recruitment of Candidates.Click Here TO REGISTER" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute - MBBS Admission, Eligibility Criteria"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/admission" }
            ]
        }
    ],
    "review": [
        {
            type: 'title',
            title: "Andijan State Medical Institute, Uzbekistan - Reviews"
        },

        {
            "type": "list-bollet",
            "title": "Reviews on Andijan State Medical University",
            "info": "Here are some of the reviews by students studying at the university at the moment:",
            "data": [
                "<b>Rajeshwari Bhirad</b><p>I really enjoyed my time at Andijan State Medical University. The instructors were beneficial and instructive.</p>",
                "<b>Saurabh Yadgire</b><p>Everyone looking for a top-notch medical education should give the Andijan State Medical University some serious thought.</p>",
                "<b>Rhythm Chandak</b><p>The Andijan State Medical University provides excellent amenities and a welcoming staff. Throughout my entire stay, I received wonderful care.</p>",
            ]
        },

        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute, Uzbekistan - Reviews" },
                { name: "description", content: "Andijan State Medical Institute offers the best and affordable MBBS education. click here to see reviews" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute, Uzbekistan - Reviews"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/reviews" }
            ]
        }
    ],
    "placement": [
        {
            type: 'title',
            title: "Andijan State Medical Institute, Uzbekistan - Placements"
        },

        {
            "type": "list-bollet",
            "title": "Andijan State Medical Institute Placements",
            "info": "",
            "data": [
                "Graduates might return to India and seek work after completing their various degrees.",
                "Internships and jobs in and around Uzbekistan are encouraged for students at the Andijan State Medical University.",
            ]
        },

        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute, Uzbekistan - Placements" },
                { name: "description", content: "Placement in the college is best because it has a record of 100% placement, check top recruiters" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute, Uzbekistan - Placements"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/placement" }
            ]
        }
    ],
    "gallery": [
        {
            type: 'title',
            title: "Andijan State Medical Institute, Uzbekistan - Gallery"
        },

        {
            "image": [
                {
                    "type": "image",
                    "data": "../../assets/img/university_gallery/andijan-state-medical-institute-building.jpg",
                    "alt": "Andijan State Medical Institute Building"
                },
                {
                    "type": "image",
                    "data": "../../assets/img/university_gallery/andijan-state-medical-institute-computer-lab.jpg",
                    "alt": "Andijan State Medical Institute Computer Lab"
                },
                {
                    "type": "image",
                    "data": "../../assets/img/university_gallery/andijan-state-medical-institute-conferencce-hall.jpg",
                    "alt": "Andijan State Medical Institute Conferencce Hall"
                },
                {
                    "type": "image",
                    "data": "../../assets/img/university_gallery/andijan-state-medical-institute-sports-activities.jpg",
                    "alt": "Andijan State Medical Institute Sports Activities"
                },
                {
                    "type": "image",
                    "data": "../../assets/img/university_gallery/andijan-state-medical-institute-students.jpg",
                    "alt": "Andijan State Medical Institute Students"
                }
            ],
            "video": [
                // {
                //     "type": "video",
                //     "data": "../../assets/img/University of Debrecen _ Campus Tour _ Study in Top University of Hungary.mp4",
                //     "alt": "Image 1"
                // },
                
            ]
        },
      

        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute, Uzbekistan - Gallery" },
                { name: "description", content: "Andijan State Medical Institute was established in 1955. ranks 57 in Uzbekistan. check gallery here" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute, Uzbekistan - Gallery"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/gallery" }
            ]
        }
    ],
    "scholarship": [
        {
            type: 'title',
            title: "Andijan State Medical Institute - Scholarships Details"
        },
        {
            "type": "list-bollet",
            "title": "Andijan State Medical Institute Scholarships",
            "info": "",
            "data": [
                "Scholarships for Indians - International Scholarships",
                "UWA International Scholarships",
                "Entrance Scholarship for Board Courses",
            ]
        },

        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute - Scholarships Details" },
                { name: "description", content: "Andijan State Medical Institute offers Scholarships Opportunities. check scholarship and bursary" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute - Scholarships Details"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/scholarship" }
            ]
        }
    ],
    "faculty": [
        {
            type: 'title',
            title: "Andijan State Medical Institute, Uzbekistan - Faculties"
        },

        {
            "type": "table",
            "title": "Andijan State Medical Institute Faculty",
            "info": "",
            "col": [
                "Name",
                "Designation"
            ],
            "row": [
                {
                    "Name": "Madazimov Madamin Muminovich",
                    "Designation": "Rector and Professor"
                },
                {
                    "Name": "Salakhidinov Kamoliddin Zukhriddinovich",
                    "Designation": "Pro-Rector and Professor"
                }
            ]
        },

        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute, Uzbekistan - Faculties" },
                { name: "description", content: "Browse list of faculties of Andijan State Medical Institute. click here for departments" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute, Uzbekistan - Faculties"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/faculty" }
            ]
        }
    ],
    "news": [
        {
            type: 'title',
            title: "Andijan State Medical Institute - News & Articles"
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute - News & Articles" },
                { name: "description", content: "Read latest News, Articles of Ago medical and educational center about Cutoffs, Placements, courses, Fees. check all the news and articles here" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute - News & Articles"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/news-and-articles" }
            ]
        }
    ],
    "hostel": [
        {
            type: 'title',
            title: "Andijan State Medical Institute - Hostel & Accommodation"
        },
        {
            "type": "list-bollet",
            "title": "Hostel & Accommodation",
            "info": "Following are the hostel facilities at Andijan State Medical Institute:",
            "data": [
                "The rooms of the hostels of Andijan State Medical Institute are well furnished, comfortable with excellent security facilities.",
                "Each dormitory has its own study and computer rooms, kitchen, and restrooms.",
                "At the hostels, all the facilities are provided including the centralised heating system, hot and cold water.",
                "For the comfort of the students, modern LCD televisions, washing machines and furniture are available.",
                "Hostel’s kitchens are equipped with refrigerators and gas stoves for students who want to cook their own meals.",
                "The Internet room is available at the hostel that is equipped with modern computers and all conditions for work with the Internet system.",
                "In the hostel, there is a domestic service house, with a barbershop, a sewing room, and a photographic salon.",
                "A student’s gym is available at the hostel equipped with all the necessary facilities.",
                "The reading room of the hostel is equipped with 23 types of newspapers and 16 kinds of magazines, also 1220 educational literature.",
            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute - Hostel & Accommodation" },
                { name: "description", content: "accomodation inquiry Grab this opportunity to make a bright future· Click Here to know about accomodation" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute - Hostel & Accommodation"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/hostel" }
            ]
        }
    ],
    "faq": [
        {
            type: 'title',
            title: "Andijan State Medical Institute - Important FAQs"
        },
        {
            "type": "question-list",
            "title": "Andijan State Medical Institute - Important FAQs",
            "info": "Apart from the information provided, here are some frequently asked questions on Quora and Careers 360:",
            "data": [
                {
                    "ques": "Q. How are the library facilities at the Andijan State Medical Institute?",
                    "ans": "The library of the Andijan State Medical Institute has approximately 286000 books. The library consists of textbooks, tutorials, reference books, periodicals, dictionaries, manuals, etc. for research and studies. In the library, there are 5 reading halls and the newest collection of articles on medicine in the electronic library."
                },
                {
                    "ques": "Q. Does the university arrange any activities for international students?",
                    "ans": "Yes, the administration of Andijan State Medical Institute arranges trips for international students staying in the dormitory of the Institute around the city of Andijan. Foreign students participate in the activities such as New Year celebrations, exploring the city, meeting the hospitable Andijan residents."
                },
                {
                    "ques": "Q. Are there any post graduate courses available in the university?",
                    "ans": "Yes, the post graduate courses available in the university are in the subjects of Anatomy, ENT, Cardiology, Dermatology and Venereology, Pharmacology, Obstetrics and Gynecology, X-ray Diagnostics, Neurosurgery, Psychiatry and pediatrics, etc"
                },
                {
                    "ques": "Q. Is there any international airport available near the university?",
                    "ans": "Andijan airport is the nearest airport to Andijan State Medical Institute."
                },
                {
                    "ques": "Q. Is there any associated clinic/hospital to the Andijan State Medical Institute?",
                    "ans": "Yes. There is an Institute Clinic that has a capacity of 700 beds. It is equipped with modern diagnostic equipment (MRT, doppler, Scanners, endoscopes etc.) and has affiliations of republican Centers on Vertebrology, Lymphology and Cardiac Surgery."
                },
                {
                    "ques": "Q. Do I need to give any entrance exam for taking admission into Andijan State Medical Institute?",
                    "ans": "No, there is no entrance exam, but you must clear the NEET exam for admission into Andijan State Medical Institute."
                },
                {
                    "ques": "Q. Is the degree from Andijan State Medical Institute recognized in India?",
                    "ans": "Yes, the degree from Andijan State Medical Institute is recognized in India."
                },
            ]
        },
        {
            type: "meta",
            meta: [
                { name: "title", content: "Andijan State Medical Institute - Important FAQs" },
                { name: "description", content: "Andijan State Medical Institute is one of the leading institute situated in the ancient cities of Fergana Valley, Andijan City. click here to see FAQs" },
            ],
        },
        {
            type: "meta-title",
            title: "Andijan State Medical Institute - Important FAQs"
        },
        {
            type: "meta-links",
            link: [
                { rel: "canonical", href: "https://www.selectyouruniversity.com/university/andijan-state-medical-institute/faq" }
            ]
        }

    ]
}
